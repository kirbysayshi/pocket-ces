...
var store = createStore(function (state, action) {
  ...
  if (action.type === 'input-thrust-system') {
    action.keys.forEach(function (key) {
      inputThrustSystem(key,
        state.components['verlet-position'][key],
        state.components['rotation'][key],
        state.components['thrust'][key],
        state.components['keyboard-state'][0] // not real. TODO: make non-hydratable state have a special id?
      )
    })
  }
...
function inputThrustSystem (key, position, rotation, thrust, keyboard) {
  if (keyboard.down.UP) {
...
function inputThrustSystem (state, action) {
  // 'verlet-position', 'rotation', 'thrust', 'human-controlled-01'
  // keyboard-state
  var ids = query(state, ['verlet-position', 'rotation', 'thrust', 'human-controlled-01']);
  for (var i = 0, id; id = ids[i], i < ids.length; i++) {
    var position = state.components['verlet-position'][id];
    var rotation = state.components['rotation'][id];
    var thrust = state.components['thrust'][id];
